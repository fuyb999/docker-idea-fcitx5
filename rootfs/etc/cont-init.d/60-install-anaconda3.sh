...
# [for docker exec] 调用 conda init 初始化conda 在 /root/.bashrc 添加初始化脚本 自动激活conda base
#chmod -R +x $ANACONDA_HOME && $ANACONDA_HOME/bin/conda init bash
# [for docker idea terminal] 在$HOME/.bashrc 添加初始化脚本 在idea terminal中自动激活conda base ($HOME=/config 而不是 /root,所以两个地方都需要初始化)
